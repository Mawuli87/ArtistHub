...
import com.messieyawo.artistshub.databinding.ItemArtistAlbumTrackBinding
import com.messieyawo.artistshub.models.artistAlbums.artistAlbumData


class SingleArtistAlbumAdapter(context: Context,
                               val list: ArrayList<artistAlbumData>,
                               private val onAlbumItemClickedListener:OnItemClicked
) :
    RecyclerView.Adapter<SingleArtistAlbumAdapter.VH>() {
    class VH(val binding: ItemArtistAlbumTrackBinding) : RecyclerView.ViewHolder(binding.root)

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): VH = VH(
        ItemArtistAlbumTrackBinding.inflate(LayoutInflater.from(parent.context), parent, false)
    )
